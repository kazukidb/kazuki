...
public class KeyValueStoreJdbiH2Impl extends KeyValueStoreJdbiBaseImpl {
  ...
  @Inject
  public KeyValueStoreJdbiH2Impl(IDBI database, SchemaStore schemaManager, SequenceService sequences,
      KeyValueStoreConfiguration config) {
    this(database, schemaManager, sequences, config.getGroupName(), config.getStoreName(), config
        .getPartitionName());
  }

  public KeyValueStoreJdbiH2Impl(IDBI database, SchemaStore schemaManager, SequenceService sequences,
      String groupName, String storeName, String partitionName) {
    super(database, schemaManager, sequences, groupName, storeName, partitionName);
  }
}
